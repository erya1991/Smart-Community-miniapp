...
<template>
  <AppPage :status="status" :state-message="errorMessage" secondary @retry="load">
    ...
    <view class="operational-head"><view><text class="operational-head__title">所属主体门店</text><text class="operational-head__note">门店是经营和履约场所，不独立取得主体资格</text></view><view v-if="permissions.canCreate" class="head-action"><AppButton size="compact" @click="openSubPage('/pages/stores/editor/index')">新增门店</AppButton></view></view>
    <view v-if="data" class="operational-list">
      <BaseCard v-for="store in data" :key="store.id" class="object-card">
        ...
        <view class="object-card__actions"><view v-if="permissions.canEdit" class="compact-action" hover-class="compact-action--pressed" @click="openSubPage(`/pages/stores/editor/index?id=${store.id}`)">编辑</view><view v-if="permissions.canDisable" class="compact-action" :class="{ 'compact-action--danger': store.status === 'active' }" hover-class="compact-action--pressed" @click="changeStatus(store.id, store.status === 'active' ? 'disabled' : 'active')">{{ store.status === 'active' ? '停用' : '恢复' }}</view></view>
      </BaseCard>
    </view>
  </AppPage>
</template>
